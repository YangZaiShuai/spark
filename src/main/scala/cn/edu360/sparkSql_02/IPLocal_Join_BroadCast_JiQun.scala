...
//根据ip求归属地
object IPLocal_Join_BroadCast_JiQun {
  def main(args: Array[String]): Unit = {
   ...
   // val ips = session.read.textFile("data/ip.txt")
    val ips = session.read.textFile(args(0))
   ...
   val sql =  session.sql("select ip2Province(num) as province,count(*) sums from access group by province order by sums desc")
    //写到本地mysql
    val url = "jdbc:mysql://192.168.9.1:3306/my1?characterEncoding=utf-8"
    val table_name = "access_log"
    val p= new Properties()
      p.setProperty("user","root")
      p.setProperty("password","123456")
      p.setProperty("driver","com.mysql.jdbc.Driver")
    sql.write.mode(SaveMode.Overwrite).jdbc(url,table_name,p)
