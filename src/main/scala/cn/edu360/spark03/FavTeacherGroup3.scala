...
object FavTeacherGroup3 {
  def main(args: Array[String]): Unit = {
    ...
    val reduce: RDD[((String, String), Int)] = AndOne.reduceByKey(_+_)  //发生了一次shuffle

    val AllSubs: Array[String] = data.map(_._1).distinct().collect()

    val by: RDD[((String, String), Int)] = reduce.partitionBy(new Mypartitioner(AllSubs)) //一次shuffle

    val partitions = by.mapPartitions({
      it =>
        it.toList.sortBy( -_._2).take(topK).iterator
    })
    partitions.saveAsTextFile("out4")

  }
}

class Mypartitioner(sub:Array[String]) extends Partitioner{
